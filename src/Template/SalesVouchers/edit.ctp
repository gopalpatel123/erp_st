								...
								?>
									<tr class="MainTr" row_no="<?php echo $i;?>">
											...
											?>
												...
												?>
													<tr>
														...
														?>
															...
															{
																echo $this->Form->input('sales_voucher_rows.'.$i.'.reference_details.'.$j.'.ref_name', ['options'=>$refDropDown[$sales_voucher_row->id],'label' => false,'class' => 'form-control input-sm paymentType refList','required'=>'required','value'=>$reference_detail->ref_name]);
														...
														<td width="20%" style="padding-right:0px;vertical-align: top !important;">
															...
															echo $this->Form->input('sales_voucher_rows.'.$i.'.reference_details.'.$j.'.'.$name, ['label' => false,'class' => 'form-control input-sm calculation numberOnly rightAligntextClass','placeholder'=>'Amount','required'=>'required','value'=>$value,'type'=>'text']); ?>
														</td>
												...
												<tfoot>
												    <tr class="remove_ref_foot">
														...
														<td><input type="text" class="form-control input-sm rightAligntextClass total calculation ttl noBorder"  value="<?php echo $sales_voucher_row->total;?>" name="sales_voucher_rows[<?php echo $i;?>][total]" readonly></td>
										...
										<?php if(empty($sales_voucher_row->debit))
											...
											?>
											<?php echo $this->Form->input('debit', ['label' => false,'class' => 'form-control input-sm  debitBox rightAligntextClass numberOnly totalCalculation','placeholder'=>'Debit','value'=>$sales_voucher_row->debit,'style'=>@$style1]); ?>
										...
										?>
											<?php echo $this->Form->input('credit', ['label' => false,'class' => 'form-control input-sm  creditBox rightAligntextClass numberOnly totalCalculation','placeholder'=>'Credit','value'=>$sales_voucher_row->credit,'style'=>@$style2]); ?>
										</td>
		...
		$(document).ready(function() {
			...
			$('.ledger').die().live('change',function()
			{
				...
				if(openWindow=='party'){
					...
					windowContainer.html('<table width=90% class=refTbl><tbody></tbody><tfoot><tr style=border-top:double#a5a1a1><td colspan=2></td><td>$total_input</td><td $style>$total_type</td></tr></tfoot></table><a role=button class=addRefRow>Add Row</a>');
			...
			function renameBankRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					...
					$(this).find('td:nth-child(2) input.cheque_no').attr({name:'sales_voucher_rows['+row_no+'][cheque_no]',id:'sales_voucher_rows-'+row_no+'-cheque_no'}).rules('add','required');
					$(this).find('td:nth-child(3) input.cheque_date').attr({name:'sales_voucher_rows['+row_no+'][cheque_date]',id:'sales_voucher_rows-'+row_no+'-cheque_date'}).datepicker().rules('add','required');
					if(type=='Cheque')
					{ 
			...
			function renameRefRows(SelectedTr){
				...
				SelectedTr.find('td:nth-child(2) div.window table tbody tr').each(function(){
					...
					if(is_select){
						...
						$(this).find('td:nth-child(5) input.dueDays').attr({name:'sales_voucher_rows['+row_no+'][reference_details]['+i+'][due_days]',id:'sales_voucher_rows-'+row_no+'-reference_details-'+i+'-due_days'});
					}
