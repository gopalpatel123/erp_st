...
class GrnsController extends AppController
{
    ...
    public function index()
    {
        ...
        $grns = $this->paginate($this->Grns->find()
			->contain(['GrnRows'])->matching(
				'GrnRows.Items', function ($q) use($where1) {
		...
		]])->group(['Grns.id'])->order(['Grns.id'=>'DESC']));
		
		//$grnsDatas=$this->paginate($this->Grns->find()->contain(['GrnRows'])->where(['Grns.company_id'=>$company_id,'Grns.financial_year_id'=>$financialYear_id,'Grns.status'=>'Pending','Grns.location_id'=>$location_id]));
		/* foreach($grns as $grnsData){ pr($grnsData->id); exit;
			$ItemLedger = $this->Grns->ItemLedgers->find()->where(['ItemLedgers.item_id'=>$grn_row->item_id,'ItemLedgers.location_id'=>$grnsData->location_id]);
			
		} */
		
		$grnToBeCreateVoucher=[];
		foreach($grns as $grnsData){
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) 
		{
			...
			$Voucher_no = $this->Grns->find()->select(['voucher_no'])->where(['Grns.company_id'=>$company_id,'Grns.financial_year_id'=>$financialYear_id])->order(['voucher_no' => 'DESC'])->first();
			if($Voucher_no)
			{
			...
			$grn->financial_year_id =$financialYear_id;
